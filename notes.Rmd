...
So all transformations must happen first before clip-paths are applied.  That
actually works better, because we can then easily compute the extents before
clipping which is what we're supposed to do anyway.
...
* `approximate_fill` should become `approximate_color` and for gradients should
  approximate the opacity too, not just the color.
